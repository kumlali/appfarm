...
# ------------------------------------------------------------------------------
# Writes given string to console and given file.
#
# Parameters:
#   $1: String to be logged.
#   $2: The file that the log is added.
# ------------------------------------------------------------------------------
logToConsoleAndFile () {
...
getJavaVersion () {
  ${CONF_JAVA_HOME}/bin/java -version 2>&1 | awk 'NR==1{ gsub(/"/,""); print substr($3,1,3)}'
}


getJavaVendor () {
  ${CONF_JAVA_HOME}/bin/java -version 2>&1 | awk 'NR==3{ gsub(/"/,""); print $1 " " $2 }'
}
