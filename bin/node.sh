...
# ------------------------------------------------------------------------------
# Manages all the instances on the current machine(node).
#
...
# ------------------------------------------------------------------------------
# Moves latest deployment artifacts('app.zip', 'node.zip') in 
...
archive () {
  ...
  log "Archiving latest deployment artifacts('app.zip', 'node.zip') under '${latest_dir}' to '${previous_dir}' ..."
...
# ------------------------------------------------------------------------------
# Moves previous deployment artifacts('app.zip', 'node.zip') in
# ${CONF_APP_HOME}/deploy/previous to ${CONF_APP_HOME}/deploy/latest
# directory.
# ------------------------------------------------------------------------------
restore () {
  ...
  log "Restoring archived deployment artifacts('app.zip', 'node.zip') under '${previous_dir}' to '${latest_dir}' ..."
...
#
# 'node.zip' contains application's configuration files, farm configuration,
...
usage () {
  ...
  \n                  deployment directory(latest) to archive . \
  \n\n  deploy        : Deploys 'node.zip' to the node, then deploys 'app.zip' to all \
  \n                  the instances of the node parallelly. \
  ...
  \n                  fails to start. \
  \n\n  restore       : Moves archived deployment artifacts('app.zip', 'node.zip') \
  \n                  under 'previous' directory to deployment directory (latest). \
  \n                  You need to execute 'deploy' command to deploy them. \
