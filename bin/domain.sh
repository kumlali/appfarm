...
# ------------------------------------------------------------------------------
# Manages all the instances(applications) on all the machines(nodes).
#
...
# ------------------------------------------------------------------------------
# Deploys 'node.zip' to the node, then deploys 'app.zip' to all the instances
# of the node parallelly.
# ------------------------------------------------------------------------------
deploy () {
...
# ------------------------------------------------------------------------------
# Deploys 'node.zip' to the node, then deploys 'app.zip' to all the instances of
# the node parallelly. Then, passes to the next node for the same operation.
#
...
# ------------------------------------------------------------------------------
# Moves latest deployment artifacts('app.zip' and 'node.zip') 
...
# ------------------------------------------------------------------------------
# Moves archived deployment artifacts('app.zip' and 'node.zip') under 'previous' 
# directory to deployment directory (latest) on all the nodes. You need to
# execute 'deploy' command, to deploy them
# ------------------------------------------------------------------------------
restore () {
...
usage () {
  ...
  \n\n./domain.sh <command> \
  \n\nCommands: \
  \n\n  archive       : Moves latest deployment artifacts('app.zip', 'node.zip') under \
  \n                  deployment directory(latest) to archive on all the nodes parallelly.\
  \n\n  deploy        : Deploys 'node.zip' to the node, then deploys 'app.zip' to all the \
  \n                  instances of the node parallelly. These operations are executed on \
  \n                  the nodes parallelly. \
  \n\n  deploy-ha     : Deploys 'node.zip' to the node, then deploys 'app.zip' to all the \
  \n                  instances of the node parallelly. These operations are executed on \
  \n                  all the nodes sequentially. To achive this, it deploys applications \
  \n                  and artifacts on a node, waits for a while, then passes to the next \
  \n                  node for the same operation. Therefore, there will be no service \
  \n                  interruption. \
  ...
  \n\n  restore       : Moves archived deployment artifacts('app.zip', 'node.zip') under \
  \n                  'previous' directory to deployment directory (latest) on all the \
  \n                  nodes. You need to execute 'deploy' command to deploy them. \
